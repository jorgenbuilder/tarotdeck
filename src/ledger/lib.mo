...
module {
    ...
    public class Ledger (state : Types.State) {
        ...
        let ledger = HashMap.fromIter(ledgerItems.vals(), 0, ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash)

        // I found that this was necessary to interoperate with Stoic
        private let extMetaData : Types.ExtMetadata = #nonfungible({ metadata = ?[Blob.fromArray([])]; }); 
        ...
        public shared query func metadata (token : ExtCore.TokenIdentifier) : async Result.Result<Types.ExtMetadata, ExtCore.CommonError> {
